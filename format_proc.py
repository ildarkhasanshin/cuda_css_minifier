...
def min_css(do_format):
    ...
    min_css_path = ed.get_filename().replace('.css', '.min.css')
    if os.path.isfile(min_css_path):
        res = msg_box('File ' + min_css_path + ' already exists', MB_OK)
    else:
        f = open(min_css_path, 'w')
        f.write(text)
        f.close()
    file_open(min_css_path)